...
<body class="min-h-screen flex flex-col justify-between p-4 sm:p-6 antialiased">
  ...
  <script>
    ...
    const CODEF_CLIENT_ID = '${CODEF_CLIENT_ID}';
    const CODEF_CLIENT_SECRET = '${CODEF_CLIENT_SECRET}';
    const CODEF_PUBLIC_KEY = \`${CODEF_PUBLIC_KEY}\`;

    let currentProvider = 'kakao';
    let currentSessionId = 'MOB_' + Date.now();
    let currentToken = null;
    let currentTwoWayData = null;
    let timerInterval = null;

    window.addEventListener('DOMContentLoaded', () => {
      const params = new URLSearchParams(window.location.search);
      const s = params.get('session');
      if (s) currentSessionId = s;
    ...
    async function getCodefToken() {
      ...
      const auth = btoa(CODEF_CLIENT_ID + ':' + CODEF_CLIENT_SECRET);
    ...
    function encryptIdentity(plainText) {
      if (plainText.length < 13) return plainText;
    ...
    async function handleStartAuth() {
      ...
      submitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i><span>인증 요청 전송 중...</span>';
      ...
      try {
        ...
        const loginTypeLevel = currentProvider === 'pass' ? '5' : '1';
        const loginType = identity.length >= 13 ? '6' : '5';
        const sessionId = 'CRM_' + phoneNo + '_' + Date.now();
        ...
        const basePayload = {
          ...
          userName: userName,
          phoneNo: phoneNo,
          identity: encryptIdentity(identity),
          id: sessionId,
        ...
        if (loginTypeLevel === '5') {
          basePayload.telecom = telecom || '0';
        }

        const res = await fetch('https://development.codef.io/v1/kr/public/nt/etc-yearend-tax/income-tax-credit', {
          method: 'POST',
          headers: {
        ...
        const is2Way = json.data?.continue2Way || json.result?.code === 'CF-03002';
        ...
        if (is2Way) {
          ...
          showError(json.result?.message || '국세청 간편인증 요청에 실패했습니다.');
        }
      } catch (err) {
        showError('네트워크 통신 오류: ' + err.message);
      }
    }

    async function handleConfirmAuth() {
      if (!currentTwoWayData) return;
      const confirmBtn = document.getElementById('btn-confirm');
      confirmBtn.disabled = true;
      confirmBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i><span>국세청 의료비 자료 수신 중...</span>';
      ...
      try {
        ...
        // 1. Fetch Primary Year (2025)
        const twoWayPayload = {
          ...currentTwoWayData.basePayload,
          is2Way: true,
          simpleAuth: '1',
          twoWayInfo: currentTwoWayData.twoWayInfo
        };

        const res1 = await fetch('https://development.codef.io/v1/kr/public/nt/etc-yearend-tax/income-tax-credit', {
          method: 'POST',
          headers: {
            'Authorization': 'Bearer ' + currentTwoWayData.token,
            'Content-Type': 'application/json'
          },
          body: JSON.stringify(twoWayPayload)
        });

        const text1 = decodeURIComponent(await res1.text());
        const json1 = JSON.parse(text1);
        const data1 = json1.data || json1;

        if (data1 && (data1.resDeductibleList || Array.isArray(data1))) {
          yearsMap['2025'] = data1;
        }

        // 2. Fetch Year 2024 (Re-using session ID)
        try {
          const subPayload2024 = {
            ...currentTwoWayData.basePayload,
            searchStartYear: '2024'
          };
          delete subPayload2024.is2Way;
          delete subPayload2024.simpleAuth;
          delete subPayload2024.twoWayInfo;

          const res2 = await fetch('https://development.codef.io/v1/kr/public/nt/etc-yearend-tax/income-tax-credit', {
            method: 'POST',
            headers: {
              'Authorization': 'Bearer ' + currentTwoWayData.token,
              'Content-Type': 'application/json'
            },
            body: JSON.stringify(subPayload2024)
          });
          const text2 = decodeURIComponent(await res2.text());
          const json2 = JSON.parse(text2);
          if (json2.data || json2.resDeductibleList) {
            yearsMap['2024'] = json2.data || json2;
          }
        } catch (e2) {}

        // 3. Fetch Year 2023 (Re-using session ID)
        try {
          const subPayload2023 = {
            ...currentTwoWayData.basePayload,
            searchStartYear: '2023'
          };
          delete subPayload2023.is2Way;
          delete subPayload2023.simpleAuth;
          delete subPayload2023.twoWayInfo;

          const res3 = await fetch('https://development.codef.io/v1/kr/public/nt/etc-yearend-tax/income-tax-credit', {
            method: 'POST',
            headers: {
              'Authorization': 'Bearer ' + currentTwoWayData.token,
              'Content-Type': 'application/json'
            },
            body: JSON.stringify(subPayload2023)
          });
          const text3 = decodeURIComponent(await res3.text());
          const json3 = JSON.parse(text3);
          if (json3.data || json3.resDeductibleList) {
            yearsMap['2023'] = json3.data || json3;
          }
        } catch (e3) {}

        // Fallback: If yearsMap empty, use data1
        if (Object.keys(yearsMap).length === 0 && data1) {
          yearsMap['2025'] = data1;
        }

        // Upload Full Result to Supabase Storage
        const uploadPayload = {
          sessionId: currentSessionId,
          userName: currentTwoWayData.userName,
          phoneNo: currentTwoWayData.phoneNo,
          identity: currentTwoWayData.identity,
          rawNtsData: data1,
          yearsMap: yearsMap,
          completedAt: new Date().toISOString()
        };

        await fetch(SUPABASE_URL + '/storage/v1/object/' + STORAGE_BUCKET + '/hometax_' + currentSessionId + '.json', {
          method: 'POST',
          headers: {
        ...
        showError('의료비 자료 수신 오류: ' + err.message);
      }
    }
...
async function deploy() {
  ...
  console.log('[2/3] Uploading direct 2-way auth index.html...');
  const putPayload = {
    message: 'Deploy Direct Client 2-Way Auth Engine with 3-Year CODEF Support',
  ...
  if (putRes.status === 200 || putRes.status === 201) {
    console.log('🎉 Successfully deployed direct 2-way auth engine to GitHub Pages!');
  } else {
