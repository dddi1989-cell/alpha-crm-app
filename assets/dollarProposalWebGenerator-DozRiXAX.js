function L({planData:y,plannerInfo:d}){const t=y||{},h=(d==null?void 0:d.name)||"홍인기 GFSR",m=(d==null?void 0:d.org_name)||"인카금융서비스(주) WLB 본부",u=(d==null?void 0:d.phone)||"010-8397-0137",i=t.clientName||"32세남",w=t.clientAge||32,z=t.clientGender||"남";t.companyName;const x=t.productName||"무배당 백만인을 위한 달러종신보험 Plus (저해약환급금형Ⅱ)",e=t.payPeriodYears||7,r=t.monthlyPremiumUSD||585.78,f=t.exchangeRateKRW||1541.7,a=Math.round(r*f),c=Math.round(r*12*e*100)/100,g=t.deathBenefitUSD||39e3,v=Math.round(g*1.5);t.appliedRatePercent;let s=t.bonusRate1,o=t.bonusRate2,l=t.payCompleteRate,n=t.payComplete1dayRate;e===7?(s||(s=22.2),o||(o=15.9),l||(l=38.77),n||(n=99.75)):e===5?(s||(s=22.5),o||(o=11),l||(l=37.91),n||(n=98.33)):(s||(s=39.7),o||(o=0),l||(l=36.1),n||(n=120.8));const p=Math.round(c*1.2489),k=(Math.round(p*1200/1e7)/10).toFixed(2),S=(Math.round(p*f/1e7)/10).toFixed(2),$=(Math.round(p*1800/1e7)/10).toFixed(2),b=new Date().toLocaleDateString("ko-KR",{year:"numeric",month:"2-digit",day:"2-digit"}).replace(/\. /g,".").replace(/\.$/,"");return`<!DOCTYPE html>
<html lang="ko">
<head>
  <meta charset="UTF-8">
  <title>${i} 고객님 달러종신 VIP 마스터 리포트</title>
  <style>
    @page {
      size: A4 landscape;
      margin: 0;
...
<body>
    ...
    <div style="margin-top: 6mm;">
      ...
      <div style="margin-top: 16mm; background: rgba(15, 23, 42, 0.7); border: 1.5px solid rgba(255,255,255,0.15); border-radius: 16px; padding: 16px 24px; display: inline-flex; gap: 40px; align-items: center;">
        <div>
          ...
          <div style="font-size: 22px; font-weight: 900; color: #ffffff; margin-top: 2px;">${i} 고객님</div>
        </div>
        <div style="border-left: 1.5px solid rgba(255,255,255,0.1); padding-left: 40px;">
          ...
          <div style="font-size: 16px; font-weight: 800; color: #38bdf8; margin-top: 2px;">${x} / ${e} 년납</div>
        </div>
        <div style="border-left: 1.5px solid rgba(255,255,255,0.1); padding-left: 40px;">
          <div style="font-size: 11.5px; color: #64748b; font-weight: 700;">발행 일자</div>
          <div style="font-size: 14px; font-weight: 800; color: #cbd5e1; margin-top: 2px; font-mono;">${b}</div>
        </div>
      </div>
    </div>
  ...
  <div class="slide">
    ...
    <div style="display: grid; grid-template-columns: 2.3fr 1fr; gap: 16px; margin: 10px 0; flex: 1; align-items: stretch;">
      <div class="card-dark" style="display: flex; flex-direction: column; justify-content: space-between; padding: 22px;">
        <div style="display: grid; grid-template-columns: 1.1fr 0.9fr 1.3fr; gap: 16px; border-bottom: 1.5px solid rgba(255,255,255,0.1); padding-bottom: 18px;">
          <div>
            <div class="hero-number">$${r.toLocaleString()}</div>
            <div class="metric-sub">월 기본보험료</div>
          </div>
          <div>
            <div class="hero-number" style="color: #fde047;">${e} 년</div>
            <div class="metric-sub">납입기간 / ${e*12} 회</div>
          </div>
          <div>
            <div class="hero-number" style="color: #38bdf8;">$${c.toLocaleString()}</div>
        ...
        <div style="margin-top: 16px;">
          ...
          <div style="font-size: 14px; color: #cbd5e1; margin-top: 8px; line-height: 1.5;">
            가입금액 <strong style="color: #ffffff; font-size: 16px;">$${g.toLocaleString()} (10년 후 최대 150% 체증 $${v.toLocaleString()})</strong> / 특약 없음 / 피보험자 ${w} 세 ${z==="남"?"남성":"여성"}
          </div>
        </div>
      </div>
      ...
      <div style="background: linear-gradient(160deg, #0284c7, #0369a1); border-radius: 18px; padding: 22px; color: #ffffff; display: flex; flex-direction: column; justify-content: space-between; box-shadow: 0 10px 25px rgba(2, 132, 199, 0.3);">
        <div>
          ...
          <div style="font-size: 32px; font-weight: 900; margin-top: 6px; letter-spacing: -1px;">약 ${(a/1e4).toFixed(1)} 만원</div>
          <div style="font-size: 12px; color: #e0f2fe; margin-top: 4px; font-weight: 600;">설계환율 ${f.toLocaleString()} 원 적용</div>
        </div>
        <div style="border-top: 1.5px solid rgba(255,255,255,0.25); padding-top: 14px;">
          ...
          <div style="font-size: 18px; font-weight: 900; margin-top: 4px; color: #fde047;">${Math.round(a*1.1/1e4)} 만 ~ ${Math.round(a*1.3/1e4)} 만원</div>
    ...
    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 자료 : 가입제안서 p.10, p.17</div>
      <div>02 / 15</div>
    </div>
  </div>
  ...
  <div class="slide">
        ...
        <div style="background: rgba(15, 23, 42, 0.85); border: 1.5px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 18px;">
          ...
          <div style="display: flex; align-items: flex-start; margin-bottom: 12px;">
            ...
            <div>
              ...
              <p style="font-size: 12.5px; color: #94a3b8; margin-top: 2px;">${e} 년 분할 납입과 10 년 이상 비과세 유지가 가능한 자금</p>
            </div>
          </div>
    ...
    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 자료 : 가입제안서 p.1-p.3</div>
      <div>03 / 15</div>
    </div>
  </div>
  ...
  <div class="slide">
        ...
        <div class="card-dark" style="border: 2px solid #0d9488; background: rgba(13, 148, 136, 0.15); padding: 18px; display: flex; flex-direction: column; justify-content: space-between;">
          <div>
            ...
            <p style="font-size: 12.5px; color: #cbd5e1; line-height: 1.5; margin-top: 6px;">
              매월 정해진 원화(110%~130%)를 납입하면 기본보험료($${r.toLocaleString()})를 내고 남은 차액이 <strong>'원화고정납입용 추가보험료 적립액'에 달러로 자동 저축</strong>됩니다 .
            </p>
          </div>
      ...
      <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; margin-top: 4px;">
        <div class="card-dark" style="border-top: 3px solid #38bdf8; padding: 14px 16px;">
          ...
          <p style="font-size: 12px; color: #cbd5e1; line-height: 1.45; margin-top: 4px;">
            환율 등락에 흔들리지 않고 매월 약 <strong>${Math.round(a*1.1/1e4)}만 ~ ${Math.round(a*1.3/1e4)}만원</strong>으로 일정하게 통장 지출 관리
          </p>
        </div>
    ...
    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 자료 : 가입제안서 p.9, p.10, p.19, p.20</div>
      <div>04 / 15</div>
    </div>
  </div>
  ...
  <div class="slide">
    ...
    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 자료 : 국제통화기금 (IMF), 2025 년 4 분기</div>
      <div>04 / 15</div>
    </div>
  </div>
  ...
  <div class="slide">
    ...
    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 자료 : 한국은행, 한국부동산원, KRX</div>
      <div>05 / 15</div>
    </div>
  </div>
  ...
  <div class="slide">
    ...
    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 가정 : 한국은행 물가안정목표 2%</div>
      <div>06 / 15</div>
    </div>
  </div>
  ...
  <div class="slide">
    ...
    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 자료 : 가입제안서 p.17, p.19-p.20</div>
      <div>07 / 15</div>
    </div>
  </div>
  ...
  <div class="slide">
        ...
        <div>
          ...
          <div style="margin-top: 14px; background: rgba(0,0,0,0.35); border-radius: 12px; padding: 14px; font-size: 13px; space-y: 8px;">
            <div style="display: flex; justify-content: space-between;">
              ...
              <strong style="color: #ffffff;">$${g.toLocaleString()} (가입금액 100%)</strong>
            </div>
            ...
            <div style="display: flex; justify-content: space-between; border-top: 1px dashed rgba(255,255,255,0.12); padding-top: 6px; margin-top: 6px;">
              ...
              <strong style="color: #fbbf24; font-size: 15px;">$${v.toLocaleString()} (가입금액 150% 확정)</strong>
            </div>
          </div>
        </div>
      </div>
        ...
        <div>
          ...
          <div style="margin-top: 14px; space-y: 8px; font-size: 13px;">
            <div style="background: rgba(217, 119, 6, 0.2); border-radius: 8px; padding: 10px 14px; display: flex; justify-content: space-between;">
              <span>${e} 년 납입완료 시점:</span>
              <strong style="color: #fde047; font-size: 14px;">보너스 지급률 ${s.toFixed(2)}% 가산</strong>
            </div>
            <div style="background: rgba(217, 119, 6, 0.2); border-radius: 8px; padding: 10px 14px; display: flex; justify-content: space-between; margin-top: 8px;">
              ...
              <strong style="color: #fde047; font-size: 14px;">보너스 지급률 ${o>0?o.toFixed(2)+"% 추가 가산":"해당없음"}</strong>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="notice-box-blue">
      체증형 구조와 2차에 걸친 유지보너스가 결합되어, 10년 시점 환급률이 <strong>124.89% ($${p.toLocaleString()})</strong>로 강력하게 점프합니다 .
    </div>

    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 자료 : 가입제안서 p.12, p.14, p.17</div>
      <div>08 / 15</div>
    </div>
  </div>
  ...
  <div class="slide">
    <div>
      ...
      <h2 class="slide-main-title">${e} 년 납입 완료 이후 , 10 년까지가 가장 중요한 구간입니다</h2>
    </div>
    ...
    <div style="display: grid; grid-template-columns: 2.2fr 1fr; gap: 18px; align-items: center; margin: 10px 0; flex: 1;">
      <div class="card-dark" style="padding: 22px;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-end; height: 120px; padding-top: 14px;">
          <div style="text-align: center;">
            <div style="color: #94a3b8; font-size: 11px;">${e===7?"36.64%":e===5?"37.91%":"30.29%"}</div>
            <div style="font-size: 12px; color: #cbd5e1; font-weight: 700; margin-top: 45px;">5 년</div>
          </div>
          <div style="text-align: center;">
            <div style="color: #2dd4bf; font-weight: 900; font-size: 13px;">${n.toFixed(2)}%</div>
            <div style="font-size: 12px; color: #2dd4bf; font-weight: 800; margin-top: 15px;">${e} 년+1일</div>
          </div>
          <div style="text-align: center;">
            <div style="color: #60a5fa; font-weight: 900; font-size: 13px;">${e===7?"108.99%":e===5?"113.89%":"105.00%"}</div>
      ...
      <div style="display: flex; flex-direction: column; justify-content: space-between; height: 100%;">
        <div class="card-dark" style="padding: 16px;">
          ...
          <div style="font-size: 24px; font-weight: 900; color: #ffffff; margin-top: 2px;">$${c.toLocaleString()}</div>
        </div>
        <div class="card-dark" style="border: 2px solid #0d9488; background: rgba(13,148,136,0.2); padding: 16px; margin-top: 10px;">
          ...
          <div style="font-size: 28px; font-weight: 900; color: #2dd4bf; margin-top: 2px;">$${p.toLocaleString()}</div>
        </div>
        <div style="font-size: 11px; color: #ef4444; font-weight: 800; margin-top: 6px;">
          ⚠ 납입 중 해지 시 큰 손실 (${e} 년 시점 환급률 ${l.toFixed(2)}%)
        </div>
      </div>
    </div>

    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 자료 : 가입제안서 p.16, p.17</div>
      <div>09 / 15</div>
    </div>
  </div>
  ...
  <div class="slide">
    ...
    <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 20px; align-items: center; margin: 10px 0; flex: 1;">
      <div>
        ...
        <div class="hero-number" style="font-size: 48px; margin-top: 4px;">$${p.toLocaleString()}</div>
      ...
      <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; align-items: flex-end;">
        <div class="card-dark" style="text-align: center; border-top: 4px solid #0d9488; padding: 18px 12px;">
          <div style="font-size: 22px; font-weight: 900; color: #ffffff;">${k} 억원</div>
        ...
        <div class="card-dark" style="text-align: center; border: 2px solid #f59e0b; background: rgba(245,158,11,0.2); padding: 22px 12px;">
          <div style="font-size: 26px; font-weight: 900; color: #fde047;">${S} 억원</div>
          <div style="font-size: 14px; color: #fde047; font-weight: 900; margin-top: 6px;">${f.toLocaleString()} 원</div>
        ...
        <div class="card-dark" style="text-align: center; border-top: 4px solid #38bdf8; padding: 18px 12px;">
          <div style="font-size: 22px; font-weight: 900; color: #ffffff;">${$} 억원</div>
    ...
    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 자료 : 가입제안서 p.17, p.21 / 환율 시나리오</div>
      <div>10 / 15</div>
    </div>
  </div>
  ...
  <div class="slide">
    ...
    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 자료 : 가입제안서 p.2, p.15, p.18-p.21</div>
      <div>11 / 15</div>
    </div>
  </div>
  ...
  <div class="slide">
    ...
    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 자료 : 소득세법 제16조, 국민건강보험법 시행령</div>
      <div>12 / 15</div>
    </div>
  </div>
  ...
  <div class="slide">
      ...
      <div class="card-dark" style="padding: 16px; border: 2px solid #a855f7;">
        ...
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; text-align: center; font-size: 12px;">
          ...
          <div style="background: rgba(0,0,0,0.5); padding: 10px; border-radius: 10px;">
            ...
            <strong style="color: #fbbf24; font-size: 14px;">부모 (${i})</strong>
          </div>
          <div style="background: rgba(0,0,0,0.5); padding: 10px; border-radius: 10px;">
    ...
    <div class="notice-box-blue">
      상속세는 6개월 이내 현금 납부가 원칙입니다 . 달러 사망보험금 $${g.toLocaleString()}은 부동산 급매나 물납 없이 상속세를 해결하는 최고의 재원이 됩니다 .
    </div>

    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 자료 : 소득세법 16 조, 상속세및증여세법 제8조</div>
      <div>13 / 15</div>
    </div>
  </div>
  ...
  <div class="slide">
      ...
      <div class="card-dark" style="border-left: 5px solid #ef4444; padding: 22px; display: flex; flex-direction: column; justify-content: flex-start;">
        ...
        <div style="display: flex; flex-direction: column; gap: 12px; font-size: 14px; color: #cbd5e1;">
          <div style="display: flex; align-items: center;">
            ...
            <span style="font-weight: 600;">${e} 년 안에 찾아 써야 하는 단기 자금</span>
          </div>
          <div style="display: flex; align-items: center;">
    ...
    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 최종 판단은 납입여력과 세금 확인 후 결정</div>
      <div>14 / 15</div>
    </div>
  </div>
  ...
  <div class="slide">
    ...
    <div style="margin: 10px 0; display: grid; grid-template-columns: 2fr 1.1fr; gap: 18px; flex: 1;">
      <div class="card-dark" style="display: flex; flex-direction: column; justify-content: space-between; padding: 20px; font-size: 12.5px;">
        <div>
          ...
          <p style="color: #cbd5e1; margin-top: 2px;">${x} / p.1-p.23</p>
        </div>
      ...
      <div style="background: linear-gradient(145deg, #1e3a8a, #0f172a); border: 2px solid #38bdf8; border-radius: 18px; padding: 22px; display: flex; flex-direction: column; justify-content: space-between; box-shadow: 0 10px 30px rgba(56, 189, 248, 0.3);">
        <div>
          ...
          <div style="font-size: 22px; font-weight: 900; color: #ffffff; margin-top: 14px;">${h}</div>
          <div style="font-size: 13px; color: #93c5fd; font-weight: 700; margin-top: 4px;">${m}</div>
          <div style="font-size: 16px; color: #fde047; font-weight: 900; margin-top: 14px; font-mono; letter-spacing: 0.5px;">${u}</div>
        </div>
    ...
    <div class="notice-box">
      <strong>계산 기준</strong>: 물가: 100 / (1.02^경과년수) | 환율: $${p.toLocaleString()} x 시나리오 환율 | 모든 수치는 이해를 돕기 위한 시뮬레이션입니다 .
    </div>

    <div class="slide-footer">
      <div>${i} 고객님 장기 달러 자산 설계 | 작성 기준 : ${b}</div>
      <div>16 / 16</div>
    </div>
  </div>

</body>
</html>`}export{L as generateDollarProposalHtml};
