...
// This is the CORRECT full index.html that:
// 1. Preserves the EXACT same UI/HTML from the PC version's gh-pages
// 2. Adds direct Cloudflare edge triggers for Step 1 and Step 2
// 3. Keeps ALL original functions intact (sendConfirmSignal, handleConfirmAuth, startAutoConfirm, finishWithSuccess, etc.)
// 4. Uses the EXACT same Supabase relay pattern as PC version
...
<body class="min-h-screen flex flex-col justify-between p-4 sm:p-6 antialiased">
  ...
  <script>
    ...
    const EDGE_API = 'https://alpha-crm-app.pages.dev/api';
    ...
    async function loadSessionInfo() {
      ...
      const info = await sbDownload('session_' + currentSessionId + '.json');
      if (!info) return;
      if (info.clientName) {
    ...
    async function handleSubmitAuth() {
      ...
      // 2. Upload to Supabase Relay (so PC CRM can also see it)
      ...
      // 4. Direct Edge Trigger: Instantly calls CODEF Step 1 → KakaoTalk/PASS push
      //    This works even when the PC CRM program is off.
      try {
        fetch(EDGE_API + '/request-auth', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(authReqPayload)
        }).catch(function(err) { console.warn('Edge trigger error:', err); });
      } catch (e) {}
    ...
    function pollForStep2Result() {
      ...
      const iv = setInterval(async function() {
        ...
        if (count > 45) { // 45 * 2s = 90s
          clearInterval(iv);
          showError('의료비 자료 수신 시간이 초과되었습니다. ' + providerLabel() + '에서 서명을 완료하셨는지 확인 후 다시 시도해 주세요.');
          return;
        }
...
async function deploy() {
  ...
  console.log('[2/3] Uploading corrected index.html...');
  const putPayload = {
    message: 'fix: rebuild complete auth page with edge triggers and all functions intact',
  ...
  console.log('[3/3] Upload status:', putRes.status);

  if (putRes.status === 200 || putRes.status === 201) {
    console.log('🎉 Successfully deployed corrected index.html to GitHub Pages!');
    console.log('URL: https://dddi1989-cell.github.io/alpha-crm-app/');
  } else {
    console.error('❌ Failed to deploy:', JSON.stringify(putRes.data).slice(0, 300));
  }
}
